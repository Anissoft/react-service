...
import { Service, Inject } from '../react-service';
...
describe('ServiceProvider', () => {
  ...
  it('Should accept services as prop and render its children', () => {
    render(
      <ServiceProvider services={{'TEST': DependencyOfDependency}}>
        Child
      </ServiceProvider>
    );
  ...
  it('Should accept services with dependencies as prop and render its children', () => {
    render(
      <ServiceProvider services={{ 'TEST': Test, 'DEPENDENCY': Dependency, 'DependencyOfDependency': DependencyOfDependency }}>
        Child
      </ServiceProvider>
    );
      ...
      return (
        <ServiceConsumer>
          {(container) => {
            ...
            expect(container.get('DEPENDENCY').dependencyOfDependency).toBe(container.get('DependencyOfDependency'));
            expect(container.get('TEST').dependency.dependencyOfDependency).toBe(container.get('DependencyOfDependency'));
  ...
  it('Should allow Symbols as tag-names', (end) => {
    ...
    class Master {
      constructor(
        @Inject(tags.dependency) public dependency: Dep,
      ){ }
    }
  ...
  it('Should throw error if not all dependencies was passed', () => {
    const test = () => {
      render(
        <ServiceProvider services={{ 'TEST': Test }}>
          Child
        </ServiceProvider>
      );
    };  
  ...
  it('Should throw error if there is a circular dependency', () => {
    const test = () => {
      render(
        <ServiceProvider services={{ 'TEST': Test, 'DEPENDENCY': Dependency, 'DependencyOfDependency': BrokenDependencyOfDependency }}>
          Child
        </ServiceProvider>
      );
    };  
